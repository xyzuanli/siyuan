...
func TestSpinBlockDOMInputSizeLimit(t *testing.T) {
	...
	if recorder.Code != http.StatusOK {
		t.Fatalf("unexpected HTTP status code: got %d, want %d", recorder.Code, http.StatusOK)
	}
	response := &struct {
		Code int    `json:"code"`
		Msg  string `json:"msg"`
	...
	if response.Msg != "dom input exceeds the maximum permitted size" {
		t.Fatalf("unexpected response message: got %q, want %q", response.Msg, "dom input exceeds the maximum permitted size")
	}
}
